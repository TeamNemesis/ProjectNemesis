...
public class PlayerStatManager : MonoBehaviour
{
    ...
    public void AddTotalMultiDamage(float plusTotalDamage)
    {
        _totalMultiDamage += plusTotalDamage;
        onTotalDamageChange?.Invoke();

    }
    public event Action onTotalDamageChange;
    ...
    ///  드론 공격력
    /// </summary>
    private float _droneAttack = 2;
    public float droneAttack { get { return _droneAttack; } }
    public void PlusDroneAttack(float PlusDroneAttack)
    {
        _droneAttack += PlusDroneAttack;
        onDroneAttackChange?.Invoke();
    }
    public void SetDroneAttack(float droneAttack)
    {
        _droneAttack = droneAttack;
        onDroneAttackChange?.Invoke();
    }
    public event Action onDroneAttackChange;


    /// <summary>
    /// 드론 사정거리
    /// </summary>
    private float _droneAttackRange = 10f;
    public float droneAttackRange { get { return _droneAttackRange; } }
    public void AddDroneAttackRange(float PlusDroneAttackRange)
    {
        _droneAttackRange += PlusDroneAttackRange;
        onDroneAttackRangeChange?.Invoke();
    }
    public event Action onDroneAttackRangeChange;

    /// <summary>
    /// 드론 공격 주기
    /// </summary>
    private float _droneAttackTime = 1.0f;
    public float droneAttackTime { get { return _droneAttackTime; } }
    public void PlusDroneAttackTime(float PlusDroneAttackTime)
    {
        _droneAttackTime += PlusDroneAttackTime;
        onDronAttackTimeChange?.Invoke();
    }
    public event Action onDronAttackTimeChange;
    #endregion
}
