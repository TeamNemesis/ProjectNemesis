...
public class ResourceManager : MonoBehaviour
{
    ...
    void InitializeRoomPrefabDict()
    {
        ...
        foreach( var prefab in _roomPrefabs)
        {
            ...
            {
                RoomType roomType = roomComponent.RoomType;
                if (!_roomPrefabMap.ContainsKey(roomType))
                {
                    _roomPrefabMap.Add(roomType, prefab);
                }
                else
                {
                    Debug.LogWarning($"이미 {roomType} 타입의 방 프리팹이 존재합니다.");
                }
            }
            else
            {
                Debug.LogWarning($"프리팹 {prefab.name}에 Room 컴포넌트가 없습니다.");
            }
        }
    }
}