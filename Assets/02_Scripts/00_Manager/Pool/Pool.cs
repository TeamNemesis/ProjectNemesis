...
public class Pool
{
    ...
    void CreatePoolObj()
    {
        // Instantiate하면서 부모를 지정하여 하이어라키 위치를 일정하게 유지
        GameObject go = Object.Instantiate(_prefab, _parent);

        // (옵션) 부모 기준 로컬 위치/회전을 초기화하고 싶으면 아래 주석 해제
    ...
    public GameObject Pop()
    {
        ...
        // Instantiate 시 부모를 지정하여 하이어라키 위치를 통일
        GameObject newGo = Object.Instantiate(_prefab, _parent);

        // (옵션) 로컬 위치/회전을 초기화하려면 아래 주석 해제
        ...
        // 새로 생성하는 경우에도 기본적으로 활성화된 상태로 반환(원하면 비활성 후 반환하도록 변경 가능)
