...
public class DebuffHandler : MonoBehaviour
{
    ...
    [SerializeField] float _allDamageBonus;
    ...
    private IEnumerator HandleDebuff(DebuffData debuff)
    {
        ...
        while (active.remainingTime > 0f && !monster.isDead)
        {
            switch (debuff.debuffName)
            {
                case "독":
                case "과부하":
                    monster.TakeDamage(active.totalValue * Time.deltaTime * _allDamageBonus);
                    break;
            }
    ...
    public void SetAllDamageBonus(float amount)
    {
        _allDamageBonus = amount;
    }
